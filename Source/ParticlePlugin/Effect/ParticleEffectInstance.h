...
private:
  ...
  {
    plSimdVec4f m_vMinPos;
    plSimdVec4f m_vMaxPos;
    plSimdVec4f m_vInvCellSize;
    plSmallArray<plSimdVec4f, 16, plAlignedAllocatorWrapper> m_Samples;
  };

  plVec4I32 m_vNumWindSamples; // not unsigned so it can be loaded directly to a SIMD register, w contains total num samples
  mutable plUniquePtr<WindSampleGrid> m_WindSampleGrids[2];
...
private:
  ...
  plHybridArray<plParticleSystemInstance*, 8> m_ParticleSystems;
