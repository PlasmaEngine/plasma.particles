...
void plParticleTypeLight::ExtractTypeRenderData(plMsgExtractRenderData& ref_msg, const plTransform& instanceTransform) const
{
  ...
  for (plUInt32 i = 0; i < uiNumParticles; ++i)
  {
    ...
    auto pRenderData = ref_msg.m_pRenderDataManager->CreateRenderDataForThisFrame<plPointLightRenderData>(nullptr);
    ...
    float fScreenSpaceSize = plLightComponent::CalculateScreenSpaceSize(plBoundingSphere::MakeFromCenterAndRadius(pRenderData->m_vGlobalPosition, pRenderData->m_fRange * 0.5f), *ref_msg.m_pView->GetCullingCamera());
