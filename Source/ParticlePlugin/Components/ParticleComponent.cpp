...
PL_BEGIN_COMPONENT_TYPE(plParticleComponent, 5, plComponentMode::Static)
{
  ...
  PL_BEGIN_MESSAGEHANDLERS
  {
    ...
    PL_MESSAGE_HANDLER(plMsgInterruptPlaying, OnMsgInterruptPlaying),
...
void plParticleComponent::OnMsgInterruptPlaying(plMsgInterruptPlaying& ref_msg)
{
  InterruptEffect();
}
...
void plParticleComponent::SetParticleEffectFile(plStringView sFile)
{
  plParticleEffectResourceHandle hEffect;

  if (!sFile.IsEmpty())
  {
    hEffect = plResourceManager::LoadResource<plParticleEffectResource>(sFile);
  }
...
plStringView plParticleComponent::GetParticleEffectFile() const
{
  if (!m_hEffectResource.IsValid())
    return "";
...
void plParticleComponent::OnMsgExtractRenderData(plMsgExtractRenderData& msg) const
{
  ...
  {
    case plCameraUsageHint::Shadow:
    case plCameraUsageHint::Reflection:
      return;
...
void plParticleComponent::Update()
{
  ...
  if (!m_EffectController.IsAlive() && (m_OnFinishedAction == plOnComponentFinishedAction2::Restart))
  {
    ...
    {
      const plTime tDiff = plTime::MakeFromSeconds(GetWorld()->GetRandomNumberGenerator().DoubleMinMax(m_MinRestartDelay.GetSeconds(), m_MinRestartDelay.GetSeconds() + m_RestartDelayRange.GetSeconds()));
...
{
  return plRangeView<const char*, plUInt32>([this]() -> plUInt32
    { return 0; },
    [this]() -> plUInt32
    { return m_FloatParams.GetCount() + m_ColorParams.GetCount(); },
    [this](plUInt32& ref_uiIt)
    { ++ref_uiIt; },
...
void plParticleComponent::UpdatePfxTransformAndBounds()
{
