...
PL_BEGIN_DYNAMIC_REFLECTED_TYPE(plParticleBehaviorFactory_Raycast, 1, plRTTIDefaultAllocator<plParticleBehaviorFactory_Raycast>)
{
  PL_BEGIN_PROPERTIES
  {
    ...
    PL_MEMBER_PROPERTY("SlideFactor", m_fSlideFactor)->AddAttributes(new plDefaultValueAttribute(0.5f), new plClampValueAttribute(0.0f, 1.0f)),
    PL_MEMBER_PROPERTY("SizeFactor", m_fSizeFactor)->AddAttributes(new plDefaultValueAttribute(0.1f), new plClampValueAttribute(0.0f, 1.0f)),
...
void plParticleBehavior_Raycast::QueryOptionalStreams()
{
...
void plParticleBehavior_Raycast::Process(plUInt64 uiNumElements)
{
  ...
  plFloat16 fDummySize = 0.0f;
  const plFloat16* pSize = m_pStreamSize != nullptr ? m_pStreamSize->GetData<plFloat16>() : &fDummySize;
  ...
  while (!itPosition.HasReachedEnd())
  {
    ...
    if (!vLastPos.IsZero())
    {
      ...
      if (!vChange.IsZero(plMath::DefaultEpsilon<float>()))
      {
        plVec3 vDirection = vChange;

        const float fSize = plMath::Max(*pSize * m_fSizeFactor, 0.01f);
        ...
        if (m_pPhysicsModule != nullptr && m_pPhysicsModule->Raycast(hitResult, vLastPos, vDirection, fMaxLen + fSize, query))
        {
          ...
          {
            const plVec3 vTangentDir = vChange - hitResult.m_vNormal * hitResult.m_vNormal.Dot(vChange);
            const plVec3 vNormalDir = vTangentDir - vChange;

            const plVec3 vNewDir = vNormalDir * m_fBounceFactor + vTangentDir * m_fSlideFactor;
        ...
        if constexpr (false)
        {
          plDebugRenderer::DrawLineSphere(m_pPhysicsModule->GetWorld(), plBoundingSphere::MakeFromCenterAndRadius(itPosition.Current().GetAsVec3(), fSize), plColor::Red);
        }
      }
    }
